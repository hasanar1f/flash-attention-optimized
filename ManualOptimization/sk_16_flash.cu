...
void forward_kernel(const float* Q, const float* K, const float* V, const int N, const int d,
    ...
    for (int j = 0; j < Tc; j++) {
        // Load Kj, Vj to SRAM with skew
        for (int x = 0; x < d; x++) {
            int skewed_ty = ty + SKEW * tx;
            if (skewed_ty < TILE_WIDTH) {
                Kj[(ty * d) + x] = K[qkv_offset + (j * TILE_WIDTH * d) + (skewed_ty * d) + x];
                Vj[(ty * d) + x] = V[qkv_offset + (j * TILE_WIDTH * d) + (skewed_ty * d) + x];
            }
        }
        __syncthreads();

        for (int i = 0; i < Tr; i++)  {
            // Load Qi to SRAM with skew
            for (int x = 0; x < d; x++) {
                int skewed_tx = tx + SKEW * ty;
                if (skewed_tx < TILE_HEIGHT) {
                    Qi[(tx * d) + x] = Q[qkv_offset + (i * TILE_HEIGHT * d) + (skewed_tx * d) + x];
                }
            }
